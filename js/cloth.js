...
Constraint.prototype.enforce = function() {
  ...
  let v_12 = this.p2.position.clone().sub(this.p1.position.clone());
  let num = v_12.length() - this.distance;
  let frac = (num * 0.5) / v_12.length();
  let v_corr = v_12.multiplyScalar(frac);
...
function Cloth(w, h, l) {
  ...
  if (structuralSprings) {
    ...
    for (v = 1; v <= h - 1; v++){
      for (u = 0; u <= w - 1; u++){
        // Structural constraints exist between a particle and the particle to its left
        // as well as the particle below it
          let c1 = new Constraint(particles[index(u, v)], particles[index(u + 1, v)], restDistance);
          constraints.push(c1);
        }
      }
      for (v = 0; v <= h - 1; v++){
        for (u = 1; u <= w - 1; u++){
          let c2 = new Constraint(particles[index(u, v)], particles[index(u, v + 1)], restDistance);
          constraints.push(c2);
        }
      }
  ...
  if (shearSprings) {
    ...
    for (v = 0; v <= h - 1; v++){
      for (u = 1; u <= w; u++){
        let c1 = new Constraint(particles[index(u, v)], particles[index(u - 1, v + 1)], restDistance * restDistanceS);
        constraints.push(c1);
      }
    }
    ...
    for (v = 0; v <= h - 1; v++){
      for (u = 0; u <= w - 1; u++){
        let c2 = new Constraint(particles[index(u, v)], particles[index(u + 1, v + 1)], restDistance * restDistanceS);
        constraints.push(c2);
      }
    }
    // ----------- STUDENT CODE END ------------
  ...
  if (bendingSprings) {
    ...
    for (v = 0; v < h; v++){
      for (u = 0; u <= w - 2; u++){
        let c1 = new Constraint(particles[index(u, v)], particles[index(u + 2, v)], restDistance * restDistanceB);
        constraints.push(c1);
      }
    }
    ...
    for (v = 0; v <= h - 2; v++){
      for (u = 0; u < w; u++){
        let c2 = new Constraint(particles[index(u, v)], particles[index(u, v + 2)], restDistance * restDistanceB);
        constraints.push(c2);
      }
    }
...
Cloth.prototype.applyGravity = function() {
  ...
  for (let i = 0; i < particles.length; i++){
    let g = GRAVITY * MASS * -1;
    let force = new THREE.Vector3(0, g, 0);
...
Cloth.prototype.applyWind = function() {
  ...
  // ----------- Our reference solution uses 5 lines of code.
  for (let i = 0; i < particles.length; i++){
    let random = (Math.random() * 2 * windStrength) - windStrength;
    windForce.add(particles[i].position.clone().normalize().multiplyScalar(random));
  }
  // ----------- STUDENT CODE END ------------
